...
public class Pyramids {
    ...
    pp.stroke(PApplet.map(pp.getAmplitude()*4, 0, 1, 0, 255), 255, 255);  //React to the colors
        ...
        pp.beginShape();
        pp.translate(pp.width/2f, pp.height/2f);
        pp.stroke(PApplet.map(pp.getAmplitude()*10, 0, 1, 0, 255), 255, 255); 
            angle += 0.01f;
        ...
        pp.beginShape();
        pp.translate(pp.width/2f, pp.height/2f);
        pp.stroke(PApplet.map(pp.getAmplitude()*2, 0, 1, 0, 255), 255, 255);
            angle += 0.01f;
        ...
        pp.translate(pp.width/2f, pp.height/2f);
            ...
            for (float t = 0; t < pp.width; t+=0.25 ) { //incremenet every time by 0.25
                float x = (t * PApplet.cos(t)*gap); //Makes a gap
                float y = (t * PApplet.sin(t))*gap; //Gap
                pp.point(x, y); //Construts and initalizes a point at x and y
            }
            
            pp.stroke(PApplet.map(pp.getAmplitude()*2, 0, 1, 0, 255), 255, 255);
            pp.translate(1000, 0); //Positioning
            for (float t = 0; t < pp.width; t+=0.25 ) { //incremenet every time by 0.05
                float x = (t * PApplet.cos(t)*gap); //Makes a gap
